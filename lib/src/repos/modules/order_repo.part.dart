...
mixin OrderRepoMixin on _RepoCore implements OrderRepo {
  ...
  Stream<List<Order>> watchOrders({bool includeDeleted = false}) {
    ...
    return joined.watch().map((rows) {
      ...
      for (final r in rows) {
        ...
        final entry =
            map.putIfAbsent(header.id, () => (header, <OrderLineRow>[]));
  ...
  Future<Order?> getOrder(String id) async {
    final row = await (db.select(db.orders)..where((t) => t.id.equals(id)))
        .getSingleOrNull();
  ...
  Future<void> upsertOrder(Order order) async {
    await db.transaction(() async {
      ...
      await (db.delete(db.orderLines)..where((l) => l.orderId.equals(order.id)))
          .go();
  ...
  Future<String?> customerNameOf(String orderId) async {
    final row = await (db.select(db.orders)..where((t) => t.id.equals(orderId)))
        .getSingleOrNull();
    return row?.customer;
  }
  ...
  Future<void> hardDeleteOrder(String orderId) async {
    await db.transaction(() async {
      await (db.delete(db.orderLines)..where((l) => l.orderId.equals(orderId)))
          .go();
