...
mixin TrashRepoMixin on _RepoCore implements TrashRepo {
  ...
  Future<void> hardDelete(String entityType, String id) async {
    switch (entityType) {
      case 'item':
        final imageRows = await db.customSelect(
          'SELECT file_path FROM item_images WHERE item_id = ?',
          variables: [Variable.withString(id)],
        ).get();
        for (final row in imageRows) {
          try {
            final file = await const AppPathService()
                .resolveAppFile(row.data['file_path'] as String);
        ...
        try {
          final dir = await const AppPathService().itemImageDirectory(id);
          if (await dir.exists()) {
        ...
        // 🔥 RESTRICT 테이블 먼저 삭제
        await (db.delete(db.orderLines)..where((t) => t.itemId.equals(id)))
            .go();

        await (db.delete(db.purchaseLines)..where((t) => t.itemId.equals(id)))
            .go();

        await (db.delete(db.works)..where((t) => t.itemId.equals(id))).go();

        // 🔥 1. 참조 데이터 먼저 삭제
        await (db.delete(db.txns)..where((t) => t.itemId.equals(id))).go();
      ...
      case 'order':
        await (db.delete(db.orders)..where((t) => t.id.equals(id)))
            .go(); // lines는 CASCADE 가정
        break;
      case 'txn':
      ...
      case 'work':
        ...
        break;
      case 'po':
        await (db.delete(db.purchaseOrders)..where((t) => t.id.equals(id)))
            .go(); // lines CASCADE
        break;
    }
    notifyListeners();
  ...
  Future<void> _restoreItem(String id) async {
    final item =
        await (db.select(db.items)..where((t) => t.id.equals(id))).getSingle();
  ...
  Future<void> _restoreFolder(String id, {bool deep = true}) async {
    ...
    if (deep) {
      ...
      for (final item in items) {
        ...
        if (extra['l1Id'] == id || extra['l2Id'] == id || extra['l3Id'] == id) {
      ...
      final children = await (db.select(db.folders)
            ..where((t) => t.isDeleted.equals(true) & t.parentId.equals(id)))
          .get();
  ...
  Future<int> _countDescendants(String folderId) async {
    ...
    final children = await (db.select(db.folders)
          ..where(
              (t) => t.isDeleted.equals(true) & t.parentId.equals(folderId)))
        .get();
  ...
  Future<({String? name, bool willRestore})> getParentFolderInfo(
    ...
    return (name: folder.name, willRestore: folder.isDeleted == true);
  }
}
